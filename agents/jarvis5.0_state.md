...
| `agents/_host_qc.md` | QC hypervisor host KB — VMs, network, Apache routing, services |
...
| jarvis5.0 (rev24) | **QC host KB + agent consistency (vLog_v1.4.0 state)**: created `agents/_host_qc.md` — complete QC hypervisor architecture knowledge base from sosreport `sosreport-qc-mar212026-2026-03-21-rgrbvir.tar.xz`; covers host identity (OVH BHS, ASUSTeK P11C-M-10G-2T, Xeon E-2236, 128GB, dual NVMe RAID1), VM inventory (10 VMs: chihuahua, Cosmoshub-4_v1, memeUpgrade cluster, jarvis, www, testDev), network map (10.0.0.0/24 NAT, DHCP reservations, Apache vhosts, 10.0.0.10 IP anomaly), services (Apache, Redis, libvirtd, cockpit, fail2ban, NFS); patched `jarvis5.0.agent.md`: vLog section renamed/updated to vOps (vLog_v1.4.0 SHIPPED), Binary Consolidation marked SHIPPED, 8 missing rev22 skills added to Installed Skills table (gh-cli, architecture-blueprint-generator, create-architectural-decision-record, github-issues, git-flow-branch-creator, generate-custom-instructions-from-codebase, cloud-design-patterns, agentic-eval + 5 new auto-invoke rules), `_host_qc.md` added to Supporting Files; `jarvis5.0_state.md` Linked files updated with `_host_qc.md`; skills §21 Infrastructure Knowledge Base added; Growth Target + Capability Index updated |
| jarvis5.0 (rev25) | **Agentupgrade + v1.4.5 sprint pre-flight**: 3 new skills installed — `sql-optimization` (SQLite query tuning + indexing for VM metrics time-series), `sql-code-review` (SQL security + anti-pattern review for new DB schema), `create-technical-spike` (time-boxed spike docs for go-libvirt SSH tunnel architecture); vOps section in agent def upgraded: v1.4.5 React SPA context documented (Vite + React 18 + TypeScript, BASE_URL prefix pattern, InvestigateModal SSE format, VM Manager via go-libvirt SSH tunnel, UFW `vops` service user); 2 new auto-invoke rules added (sql-code-review+sql-optimization on DB changes, create-technical-spike on new Go integrations); critical bug documented+fixed: `intel.go:279` nil-pointer when `GetIPAccount` returns `(nil,nil)` for new IPs (`if err != nil || acc == nil`); feature sprint scope: IP Accounts UX, Dashboard fleet panel, UFW service user, Fleet expansion, VM Manager Tier 1 (go-libvirt); deployment topology locked: vOps stays on www-VM, go-libvirt connects via SSH tunnel to `10.0.0.1` (virbr0 gateway = qc hypervisor) |
