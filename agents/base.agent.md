...
- **TI key redaction sentinel (wizard/settings snapshot)**: When exposing config to a browser UI (e.g., `/api/config/current`), replace all secret values (API keys, password hashes) with the literal string `[REDACTED]`. On save/apply, detect `[REDACTED]` sentinel and preserve the existing stored value instead of overwriting with the sentinel text. This pattern avoids wiping secrets on every settings round-trip. Applied in `internal/configwizard/web.go` (CurrentSnapshot) + `web_apply.go` (SaveVLog handler): `if val == "[REDACTED]" { val = existingVal }`.
- **Host KB file pattern (`_host_<name>.md`)**: For each physical/virtual host managed by the fleet, create `agents/_host_<shortname>.md` as a structured knowledge base: Host Identity table → Hardware → Network interfaces + DHCP table → VM Inventory table → Reverse proxy routing table → Services → Security posture → Open questions/anomalies. Suffix-prefix with `_` so it sorts near `base.agent.md`. Load on demand with `load <name>-host`. Source: sosreport (`sos report` or equivalent), parsed manually or via agent. Reference in `jarvis5.0_state.md` Linked files + agent definition Supporting Files.
- **Makefile cross-compile + deploy pattern (v1.4.0)**
